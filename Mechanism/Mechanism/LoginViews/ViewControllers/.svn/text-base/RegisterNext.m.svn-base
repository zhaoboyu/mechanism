...
@implementation RegisterNext

@synthesize codeString;
@synthesize  thirdID;

@synthesize acountString;
...
-(void)getResourceList3{
    ...
    [request addPostValue:acountString  forKey:@"phone"];
...
-(void)getResourceList{
    ...
    [request addPostValue: _codeText.text  forKey:@"code"];
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    NSLog(@"login%@",reString);
    if ([[parser objectWithString:reString ]  isKindOfClass: [NSDictionary class]])
        
    {
        NSDictionary *dic = [parser objectWithString:reString ];
        ...
        if ([[dic objectForKey:@"status"] intValue]==0) {
            ...
            [DataManager setNSString:[userDict objectForKey:@"key"]  forKey:@"userKey"];
             ...
             [self performSegueWithIdentifier:@"RegOrg" sender:nil];
//            [self dismissViewControllerAnimated:YES completion:nil];

            
//            [DataManager writeDictionary:<#(NSDictionary *)#> ToFile:<#(NSString *)#>]
//            [Util showMesssage:[dic objectForKey:@"msg"]];
...
//        ChooseCountry *vc = segue.destinationViewController;
//        vc.delegate =self;
//    }
